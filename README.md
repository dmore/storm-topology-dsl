...
must also be called to effectively declare the spout[^x-method-fail].
...
[^x-method-fail]: Making the `x`-method do two things sounds like bad design (and it is), however it's necessary because the underlying Storm topology doesn't allow setting this value after declaring a spout or bolt.
